...
func createMethod(tokens [][]SyntaxToken, i, j int, annotations []Annotation) (Method, int) {
	...
	case "public", "private", "protected":
		method.visibility = Visibility{visibility: Keyword{name: tokens[i][j]}}
		j++
	default:
		method.visibility = Visibility{visibility: Keyword{name: SyntaxToken{kind: enums.WORD_KIND, value: "protected"}}}
	}
...
func createBloc(tokens [][]SyntaxToken, i int) (Bloc, int) {
    fmt.Println("createBloc -> tokens[i] : ", tokens[i])
    bloc := Bloc{}
    j := 0
    for i < len(tokens) {
        for j < len(tokens[i]) {
...
func createAttribute(tokens [][]SyntaxToken, i, j int, annotations []Annotation) (Attribute, int) {
	...
	switch string(tokens[i][j].value) {
	case "public", "private", "protected":
		attribute.visibility = Visibility{visibility: Keyword{name: tokens[i][j]}}
		j++
	default:
		attribute.visibility = Visibility{visibility: Keyword{name: SyntaxToken{kind: enums.WORD_KIND, value: "protected"}}}
	}
	if tokens[i][j].value == "static" {
